        ...
        results.data.forEach((row) => {
          ...
          if (["BUY", "DEPOSIT", "SELL", "WITHDRAWAL"].includes(type)) {
            ...
            if (!newPortfolio.categories[tx.symbol]) {
              ...
              if (tx.symbol === "$$CASH_TX")
                newPortfolio.categories[tx.symbol] = "Cash";
              else if (
                tx.symbol.includes("-USD") &&
                available.includes("Crypto")
              )
                newPortfolio.categories[tx.symbol] = "Crypto";
              else if (
                (tx.symbol.includes(".DE") || tx.symbol.includes(".PA")) &&
                available.includes("Stock")
              )
                newPortfolio.categories[tx.symbol] = "Stock";
    ...
    <section id="import-view" style={{ paddingBottom: "100px" }}>
      ...
      <div className="card">
        <p className="muted" style={{ marginBottom: "20px" }}>
          Upload your Yahoo Finance portfolio export (CSV).
        </p>
        <input
          type="file"
          ref={fileInputRef}
