...
export default function StrategyView() {
  ...
  const [fedRate, setFedRate] = useState(() => parseFloat(localStorage.getItem('wf_fedRate')) || 5.3);
  const [platformRate, setPlatformRate] = useState(() => parseFloat(localStorage.getItem('wf_platformRate')) || 6.5);
  const [marketVol, setMarketVol] = useState(() => parseFloat(localStorage.getItem('wf_marketVol')) || 16.0);
  const [swapSpread, setSwapSpread] = useState(() => parseFloat(localStorage.getItem('wf_swapSpread')) || 0.5);
    ...
    if (valBase > 0) {
      ...
      Object.entries(mix).forEach(([catName, weight]) => {
        ...
        if (!portfolio.debtCategories?.includes(catName)) {
          ...
          if (componentValue > 0) {
            exposureRepData[catName] = (exposureRepData[catName] || 0) + componentValue;
            netWorthRepData[catName] = (netWorthRepData[catName] || 0) + (valBase * (weight / weight)); // Placeholder for proper net worth distribution
          }
        }
      });
    }
  ...
  assets.forEach(a => {
    const mix = typeof a.category === 'object' && a.category !== null ? a.category : { [a.category || 'Other']: 1.0 };
    ...
    Object.entries(mix).forEach(([cat, weight]) => {
      ...
      if (!bestLeveragePerCat[cat] || weight > bestLeveragePerCat[cat].weight) {
        bestLeveragePerCat[cat] = { symbol: a.symbol, weight: weight, ter: assetTer };
      }
      // For margin rebalancing (closest to 1x)
      if (!best1xPerCat[cat] || Math.abs(weight - 1.0) < Math.abs(best1xPerCat[cat].weight - 1.0)) {
  ...
  if (portfolio.targetAllocation && Object.keys(portfolio.targetAllocation).length > 0) {
    ...
    normalizationFactor = nonDebtTargetPctSum > 0 ? (100 / nonDebtTargetPctSum) : 1.0;
    ...
    const sum_wi_over_Li = Object.values(catConfigs)
      ...
      .reduce((sum, cfg) => sum + ((cfg.weight * normalizationFactor) / cfg.leverage), 0);
    maxPossibleLeverage = sum_wi_over_Li > 0 ? (1 / sum_wi_over_Li) : 1.0;
    hasLeverageConflict = !useMargin && targetRatio > maxPossibleLeverage + 0.001;

    // 3. Solve for Total Cash Needed (S)
    const A = Object.values(catConfigs)
      .filter(cfg => !cfg.isDebt)
      .reduce((sum, cfg) => sum + ((cfg.weight * normalizationFactor) * targetRatio / cfg.leverage), 0);
    const C = Object.values(catConfigs)
      .filter(cfg => !cfg.isDebt)
      .reduce((sum, cfg) => sum + ((cfg.weight * normalizationFactor) * targetRatio * totalValueBase - cfg.currentExp) / cfg.leverage, 0);
    ...
    const allKnownCategories = new Set([...Object.keys(portfolio.targetAllocation), ...Object.keys(exposureRepData)]);
    ...
    allKnownCategories.forEach(cat => {
      ...
      const targetExp = finalTargetExposure * (config.weight * normalizationFactor);
  ...
  return (
    <main id="main-content" style={{paddingBottom: '100px'}}>
          ...
          <div style={{marginTop: '24px', borderTop: '1px solid var(--border-color)', paddingTop: '16px'}}>
            ...
            <div style={{display: 'flex', flexDirection: 'column', gap: '10px'}}>
              {allTargetCategories.map(cat => {
                const currentPct = repTotal > 0 ? ((netWorthRepData[cat] || 0) / repTotal) * 100 : 0;
      ...
      {buyPlan.length > 0 && (
        <div className="card" style={{border: '1px solid rgba(99, 102, 241, 0.3)', background: 'linear-gradient(145deg, rgba(22, 24, 33, 1) 0%, rgba(30, 34, 47, 1) 100%)'}}>
          <div className="flex-between" style={{marginBottom: '16px'}}>
            <h3 style={{marginBottom: 0, fontSize: '1rem'}}>Path to Target</h3>
            <div style={{display: 'flex', alignItems: 'center', gap: '12px'}}>
              <label className="switch-wrapper" style={{fontSize: '0.7rem', color: 'var(--text-muted)'}}>
              ...
              <span className="indicator" style={{background: 'rgba(99, 102, 241, 0.2)', color: '#818cf8', fontSize: '0.7rem'}}>{useMargin ? 'MARGIN PLAN' : 'BUY LIST'}</span>
            </div>
          </div>
          <p className="muted" style={{fontSize: '0.8rem', marginBottom: '20px'}}>
            To reach your {portfolio.targetNetExposure?.toFixed(2)}x non-debt exposure target {useMargin ? 'by borrowing funds:' : 'using your current assets:'}
          </p>

          {!useMargin && buyPlan.some(i => i.type === 'BUY' && !i.isPossible) && (
            <div style={{padding: '12px', background: 'rgba(239, 68, 68, 0.1)', border: '1px solid rgba(239, 68, 68, 0.3)', borderRadius: '8px', marginBottom: '20px'}}>
              <p style={{margin: 0, fontSize: '0.8rem', color: '#ef4444', fontWeight: 600, display: 'flex', gap: '8px'}}>
                <span>⚠️</span>
                <span>Plan Unreachable: You need at least one Leveraged ETF (&gt; {targetRatio.toFixed(2)}x) to hit this target without margin.</span>
              </p>
            </div>
          )}
          {hasLeverageConflict && (
            <div style={{padding: '12px', background: 'rgba(239, 68, 68, 0.1)', border: '1px solid rgba(239, 68, 68, 0.3)', borderRadius: '8px', marginBottom: '20px'}}>
              <p style={{margin: 0, fontSize: '0.8rem', color: '#ef4444', fontWeight: 600, display: 'flex', gap: '8px'}}>
                <span>🚫</span>
                <span>Goal Unattainable: Your target allocation can only support up to {maxPossibleLeverage.toFixed(2)}x leverage. To reach {targetRatio.toFixed(2)}x, you must add an asset with higher leverage (e.g. 3x ETF), change your allocation goal, or enable Margin Mode.</span>
              </p>
            </div>
          )}
          {!hasLeverageConflict && (
            <>
              <div style={{display: 'flex', flexDirection: 'column', gap: '16px'}}>
                {buyPlan.map(item => (
                  <div key={`${item.type}-${item.category}`} style={{
                    padding: '12px', 
                    ...
                    <div className="flex-between" style={{marginBottom: '4px'}}>
                      <strong style={{fontSize: '0.9rem', color: (item.type === 'BUY' && !item.isPossible) ? '#ef4444' : 'var(--text-main)'}}>
                        {item.category}
                        {item.isBoosted && <span style={{marginLeft: '8px', fontSize: '0.65rem', background: 'rgba(99, 102, 241, 0.2)', color: '#818cf8', padding: '2px 6px', borderRadius: '4px', verticalAlign: 'middle'}}>LEVERAGE BOOST</span>}
                      </strong>
                      <span className={item.type === 'SELL' ? 'loss' : (item.isPossible ? 'gain' : 'loss')} style={{fontWeight: 700}}>
                        {item.type === 'SELL' ? '-' : '+'}{formatCurrency(item.gap, portfolio.baseCurrency)} <span style={{fontSize: '0.7rem', fontWeight: 400, opacity: 0.8}}>exp.</span>
                      </span>
                    </div>
                    
                    {item.type === 'SELL' ? (
                      <div className="muted" style={{fontSize: '0.8rem', display: 'flex', justifyContent: 'space-between'}}>
                        <span style={{color: '#ef4444'}}>Sell all positions (Exit Category)</span>
                        <span style={{fontSize: '0.7rem'}}>0% target</span>
                      </div>
                    ) : item.isPossible ? (
                      <div style={{display: 'flex', flexDirection: 'column', gap: '4px'}}>
                        <div className="muted" style={{fontSize: '0.8rem', display: 'flex', justifyContent: 'space-between'}}>
                          <span>{useMargin ? 'Borrow & Buy' : 'Buy'} <strong>{formatCurrency(item.amount, portfolio.baseCurrency)}</strong> of {item.instrument}</span>
                          <span style={{fontSize: '0.7rem'}}>({item.weight}x leverage)</span>
                        </div>
                        {!useMargin && (item.ter > item.benchmarkTer || item.weight > 1) && (
                            <div style={{fontSize: '0.7rem', color: '#f59e0b', display: 'flex', flexDirection: 'column', gap: '4px', padding: '8px', background: 'rgba(245, 158, 11, 0.05)', borderRadius: '6px', border: '1px solid rgba(245, 158, 11, 0.2)'}}>
                              {item.ter > item.benchmarkTer && (
                                <div className="flex-between">
                                  <span>Fee Premium (TER): <strong>{(item.ter - item.benchmarkTer).toFixed(2)}%</strong></span>
                                  <span>{formatCurrency(item.amount * (item.ter - item.benchmarkTer) / 100, portfolio.baseCurrency)}/yr</span>
                                </div>
                              )}
                              {item.weight > 1 && (
                                <>
                                  <div className="flex-between">
                                    <span>Borrowing ({(item.weight - 1).toFixed(1)}x): <strong>{((item.weight - 1) * fedRate).toFixed(2)}%</strong></span>
                                    <span>{formatCurrency(item.amount * (item.weight - 1) * (fedRate / 100), portfolio.baseCurrency)}/yr</span>
                                  </div>
                                  <div className="flex-between">
                                  ...
                                  <div className="flex-between" style={{color: '#ef4444', borderTop: '1px solid rgba(239, 68, 68, 0.2)', paddingTop: '4px', marginTop: '2px'}}>
                                    ...
                                    <div style={{textAlign: 'right'}}>
                                      ...
                                      <div style={{fontSize: '0.65rem'}}>{formatCurrency(item.amount * (0.5 * (Math.pow(item.weight, 2) - item.weight) * Math.pow((categoryVols[item.category] ?? marketVol) / 100, 2)), portfolio.baseCurrency)}/yr</div>
                                    </div>
                                  </div>
                                </>
                              )}
                      ...
                      <div style={{fontSize: '0.75rem', color: '#ef4444', marginTop: '4px', fontStyle: 'italic'}}>
                        ⚠️ Impossible to reach {targetRatio.toFixed(2)}x target with {item.weight}x assets. 
                        {item.weight <= 1 ? ' Add a leveraged ETF (2x, 3x) to this category.' : ` Need higher leverage than ${item.weight}x.`}
                      </div>
                    )}
                  </div>
                ))}
              </div>
              <div style={{marginTop: '20px', padding: '12px', background: 'rgba(255, 251, 235, 0.05)', borderRadius: '6px', border: '1px dashed rgba(245, 158, 11, 0.3)'}}>
                {useMargin ? (
                  <div style={{display: 'flex', flexDirection: 'column', gap: '12px'}}>
                    <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}>
                      <p style={{margin: 0, fontSize: '0.75rem', color: '#f59e0b'}}>Total Margin Required:</p>
                      <strong style={{color: '#f59e0b'}}>{formatCurrency(buyPlan.reduce((a, b) => a + (b.amount || 0), 0), portfolio.baseCurrency)}</strong>
                    </div>
                    
                    <div style={{padding: '10px', background: 'rgba(245, 158, 11, 0.08)', borderRadius: '6px', border: '1px solid rgba(245, 158, 11, 0.2)'}}>
                      <div className="flex-between" style={{marginBottom: '8px'}}>
                        <span style={{fontSize: '0.7rem', color: '#f59e0b', fontWeight: 600}}>COST CALCULATOR</span>
                        <div style={{display: 'flex', alignItems: 'center', gap: '6px'}}>
                          <span style={{fontSize: '0.65rem', color: '#f59e0b'}}>Platform Rate:</span>
                          <input 
                            type="number" 
                            value={platformRate} 
                            onChange={e => setPlatformRate(parseFloat(e.target.value) || 0)} 
                            step="0.1"
                            style={{width: '50px', padding: '2px 4px', fontSize: '0.7rem', background: 'rgba(0,0,0,0.3)', border: '1px solid rgba(245, 158, 11, 0.3)', color: '#f59e0b', borderRadius: '4px'}}
                          />
                          <span style={{fontSize: '0.65rem', color: '#f59e0b'}}>%</span>
                        </div>
                      </div>
                      <div className="flex-between" style={{fontSize: '0.75rem', color: '#f59e0b'}}>
                        <span>Est. Annual Cost:</span>
                        <strong>{formatCurrency((buyPlan.reduce((a, b) => a + (b.amount || 0), 0) * (platformRate / 100)), portfolio.baseCurrency)}</strong>
                      </div>
                      <div className="flex-between" style={{fontSize: '0.7rem', color: '#f59e0b', opacity: 0.8, marginTop: '2px'}}>
                        <span>Monthly "Rent":</span>
                        <span>{formatCurrency((buyPlan.reduce((a, b) => a + (b.amount || 0), 0) * (platformRate / 100) / 12), portfolio.baseCurrency)}</span>
                      </div>
                    </div>

                    <div style={{display: 'flex', flexDirection: 'column', gap: '4px', borderTop: '1px solid rgba(245, 158, 11, 0.2)', paddingTop: '8px'}}>
                      <p style={{margin: 0, fontSize: '0.75rem', color: '#f59e0b', display: 'flex', justifyContent: 'space-between', opacity: 0.9}}>
                        <span>Projected Debt/Equity Ratio:</span>
                        <strong>{(buyPlan.reduce((a, b) => a + (b.amount || 0), 0) / totalValueBase * 100).toFixed(1)}%</strong>
                      </p>
                      <p style={{margin: 0, fontSize: '0.75rem', color: '#f59e0b', display: 'flex', justifyContent: 'space-between', opacity: 0.9}}>
                  ...
                  <div style={{margin: 0, fontSize: '0.75rem', color: '#f59e0b', display: 'flex', gap: '8px'}}>
                    ...
                    <span style={{flex: 1}}>
                      {buyPlan.every(i => i.isPossible) 
                        ? (useMargin 
                          ? `Margin required: ${formatCurrency(buyPlan.reduce((a, b) => a + (b.amount || 0), 0), portfolio.baseCurrency)}. Watch your liquidation price!`
                          : (
                            <div style={{display: 'flex', flexDirection: 'column', gap: '6px', width: '100%'}}>
                              <div className="flex-between" style={{borderTop: '1px solid rgba(255, 255, 255, 0.05)', paddingTop: '8px', marginTop: '4px'}}>
                                <span style={{fontSize: '0.75rem', color: '#f59e0b', fontWeight: 600}}>STRATEGY TOTAL HOLDING COST</span>
                                <div style={{display: 'flex', alignItems: 'center', gap: '6px', justifyContent: 'flex-end', whiteSpace: 'nowrap'}}>
                                  <span style={{fontSize: '0.65rem', color: '#f59e0b'}}>Interest (Fed):</span>
                                  <input type="number" value={fedRate} onChange={e => setFedRate(parseFloat(e.target.value) || 0)} step="0.1" style={{width: '45px', padding: '1px 4px', fontSize: '0.65rem', background: 'rgba(0,0,0,0.3)', border: '1px solid rgba(245, 158, 11, 0.3)', color: '#f59e0b', borderRadius: '4px'}} />
                                  <span style={{fontSize: '0.65rem', color: '#f59e0b'}}>% | Swap:</span>
                                  <input type="number" value={swapSpread} onChange={e => setSwapSpread(parseFloat(e.target.value) || 0)} step="0.1" style={{width: '45px', padding: '1px 4px', fontSize: '0.65rem', background: 'rgba(0,0,0,0.3)', border: '1px solid rgba(245, 158, 11, 0.3)', color: '#f59e0b', borderRadius: '4px'}} />
                                  <span style={{fontSize: '0.65rem', color: '#f59e0b'}}>%</span>
                                </div>
                              </div>
                                  ...
                                  <strong style={{color: '#f59e0b', fontSize: '1.1rem'}}>
                                    {formatCurrency(
                                      Object.entries(portfolio.targetAllocation).reduce((total, [cat, targetPct]) => {
                                        ...
                                        const terDrag = (valueInLETF * (config.ter || 0) / 100) + (valueInCore * 0.07 / 100);
                                        const borrowDrag = internalDebt * (fedRate / 100);
                                        const swapDrag = internalDebt * (swapSpread / 100);
                                        const volDrag = valueInLETF * 0.5 * (Math.pow(L, 2) - L) * Math.pow(catVol / 100, 2);
                                        
                                        return total + terDrag + borrowDrag + swapDrag + volDrag;
                                      }, 0), 
                                      portfolio.baseCurrency
                                    )}/yr
                                  </strong>
                                </div>
                                <div className="flex-between" style={{opacity: 0.8, fontSize: '0.7rem', color: 'var(--text-muted)'}}>
                                  <span>Projected Net Worth / Total Exposure:</span>
                                  <span>{formatCurrency(finalNetWorth, portfolio.baseCurrency)} / {formatCurrency(finalTargetExposure, portfolio.baseCurrency)}</span>
                                </div>
                              ...
                              >
                                {showCostHelp ? 'Hide helper' : 'How to find these values?'}
                              </button>
