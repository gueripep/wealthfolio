...
export default function SettingsView() {
  ...
  const [localTargetNetExposure, setLocalTargetNetExposure] = useState(
    portfolio.targetNetExposure || 1.0,
  );
  ...
  const renameCategory = (oldName) => {
    ...
    const newCustom = portfolio.customCategories.map((c) =>
      c === oldName ? trimmed : c,
    );
    ...
    const newDebts = (portfolio.debtCategories || []).map((c) =>
      c === oldName ? trimmed : c,
    );
  ...
  const removeCategory = (catName) => {
    ...
    if (
      window.confirm(
        `Remove "${catName}"? Assets will be moved to "${portfolio.customCategories.find((c) => c !== catName) || "Other"}".`,
      )
    ) {
      ...
      const newDebts = (portfolio.debtCategories || []).filter(
        (c) => c !== catName,
      );
      const newTargetAllocation = { ...(portfolio.targetAllocation || {}) };
      if (newTargetAllocation[catName] !== undefined) {
        newTargetAllocation[fallback] =
          (newTargetAllocation[fallback] || 0) + newTargetAllocation[catName];
  ...
  const wipePortfolio = () => {
    if (
      window.confirm(
        "Are you SURE you want to wipe your entire portfolio? This will delete all transactions and cannot be undone.",
      )
    ) {
      savePortfolio({
        ...portfolio,
    ...
    <section id="categories-view">
      ...
      <div className="card" id="account-card">
        ...
        <div className="flex-between">
          <div>
            <div style={{ fontWeight: 500 }} id="account-email">
              {currentUser?.email || "—"}
            </div>
        ...
        {portfolio.customCategories.map((cat) => (
          <div className="category-row" key={cat}>
            <div style={{ display: "flex", flexDirection: "column" }}>
              ...
              <label
                className="switch-wrapper muted"
      ...
      <div className="card" style={{ marginTop: "32px" }}>
        ...
        >
          Define your ideal portfolio repartition. The total should ideally be
          100%.
        </p>
        <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
          ...
          >
            ...
            <strong
              style={{
                color:
                  Object.values(localTargets).reduce((a, b) => a + b, 0) === 100
                    ? "#10b981"
                    : "#ef4444",
              }}
            >
          ...
          {hasUnsavedTargets && (
            <button
              ...
              onClick={() => {
                savePortfolio({
                  ...portfolio,
                  targetAllocation: localTargets,
                  targetNetExposure: localTargetNetExposure,
                });
      ...
      <div
        className="card"
        style={{ marginTop: "32px", border: "1px solid #ef444433" }}
      >
        ...
        >
          This will permanently delete ALL transactions and reset your
          portfolio. This action cannot be undone.
        </p>
        <button
          className="btn"
          onClick={wipePortfolio}
