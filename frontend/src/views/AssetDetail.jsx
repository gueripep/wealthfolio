...
export default function AssetDetail({ symbol, navigate, openModal }) {
  const { portfolio, currentPrices, savePortfolio, deleteTransaction } =
    usePortfolio();

  const asset = getAssetSummary(portfolio, currentPrices, symbol);

  const isMixObj =
    asset && typeof asset.category === "object" && asset.category !== null;
  const [mixMode, setMixMode] = useState(isMixObj);

  const [mixWeights, setMixWeights] = useState(() => {
    const initial = {};
    if (isMixObj) {
      Object.entries(asset.category).forEach(([k, v]) => {
        initial[k] = (v * 100).toString();
      });
    } else if (asset) {
  ...
  const [localTer, setLocalTer] = useState(
    portfolio.assetTers?.[symbol]?.toString() || "0",
  );
  ...
  const handleRemove = () => {
    if (
      window.confirm(
        `Are you sure you want to remove ${symbol} and all its transactions?`,
      )
    ) {
  ...
  const handleMixToggle = (e) => {
    ...
    setMixMode(checked);
    if (!checked) {
      const mainCat = Object.keys(mixWeights).reduce(
        (a, b) =>
          (parseFloat(mixWeights[a]) || 0) > (parseFloat(mixWeights[b]) || 0)
            ? a
            : b,
      ...
      savePortfolio({
        ...portfolio,
        categories: { ...portfolio.categories, [symbol]: mainCat },
      });
    } else {
      applyMix(mixWeights);
    }
  ...
  const applyMix = (weights = mixWeights) => {
    ...
    savePortfolio({
      ...portfolio,
      categories: { ...portfolio.categories, [symbol]: finalMix },
    });
  };
    ...
    <section id="asset-detail-view" style={{ paddingBottom: "100px" }}>
      <header className="flex-between" style={{ marginBottom: "20px" }}>
        ...
        <button
          ...
          onClick={() => openModal("transaction", symbol)}
      ...
      <div className="card">
        ...
        {!mixMode ? (
          ...
          <div>
            ...
            >
              Set percentages for each category (e.g., 80 for 80%). Total can
              exceed 100% for leveraged ETFs.
            </p>
            {portfolio.customCategories.map((cat) => (
              <div
                key={cat}
              ...
              >
                <span style={{ fontSize: "0.9rem", width: "100px" }}>
                  {cat}
                </span>
                <input
                  type="number"
      ...
      <div className="card">
        ...
        <p className="muted" style={{ fontSize: "0.75rem", marginTop: "8px" }}>
          The Total Expense Ratio (TER) is used to calculate the fee drag in
          your rebalancing guide.
        </p>
      </div>
            ...
            .map((t, i) => (
              ...
              >
                ...
                <div style={{ textAlign: "right" }}>
                  <div>
                    {t.quantity} @ {formatCurrency(t.price, priceData.currency)}
                  </div>
                </div>
              </div>
            ))}
