...
export default function Modal({ mode, symbol, closeModal, navigate }) {
  ...
  return (
    <div className="modal-overlay" style={{display: 'flex'}} onClick={(e) => { if (e.target.className === 'modal-overlay') closeModal(); }}>
        ...
        {mode === 'asset' && (
          <div id="modal-search-step">
            ...
            <div className="input-group">
              <input type="text" value={searchQuery} onChange={handleSearch} placeholder="Search ticker (e.g. AAPL, BTC-USD)..." />
              <div id="search-results" style={{marginTop: '8px'}}>
                {searchResults.map(res => (
                  <div key={res.symbol} className="card" style={{padding: '12px', cursor: 'pointer'}} onClick={() => selectAsset(res)}>
                    <strong>{res.symbol}</strong> - {res.name} <span className="muted">({res.type})</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        {mode === 'transaction' && (
          <div id="transaction-form">
            ...
            <div className="input-group">
              ...
              <input type="number" step="any" value={txQuantity} onChange={e => setTxQuantity(e.target.value)} placeholder="0.00" />
            </div>
            <div className="input-group">
              <label className="input-label">Price per Share</label>
              <input type="number" step="any" value={txPrice} onChange={e => setTxPrice(e.target.value)} placeholder="0.00" />
            </div>
            <div className="input-group">
